# Trap 46: unsupported KV quant pairs silently fall back to CPU, and the bench tool does not error

**Found by TheTom.**

**Status: reproduced here.** Measured on two builds of the same commit differing only in the flag;
raw bench tables held outside the tree; can be produced on request, per the default in
