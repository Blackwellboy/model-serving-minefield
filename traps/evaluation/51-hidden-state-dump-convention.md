# Trap 51: hidden-state dump conventions differ, manufacturing a "final-layer norm explosion"

**Found by TheTom.**

**Status: reproduced here.** Diagnosed against the reference implementation's own source; no code
fix
was required in the implementation under test.
...
layer
   i, i.e. the **output of layer i-1**: and `hs[52]` is appended after the loop. Our dump emits
...
Runnable: [`checks/hidden_state_align.py`](../../checks/hidden_state_align.py), asserts dump counts
