# Trap 53: the fast configuration was fast because it was wrong

**Found by TheTom.**

**Status: reproduced here** (the invalidated numbers and the corrected ones came off the same box in
the same session; raw bench tables held outside the tree; can be produced on request, per
...
1. A coherence probe plus a **discriminative** probe, capital-of-France catches gross breakage, a
   ...
   [`checks/dequant_fidelity.py`](../../checks/dequant_fidelity.py).
