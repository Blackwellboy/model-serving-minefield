...
### Q100. A missing bind-mount source is fabricated as a directory, so the serve fails late and a deleted file mount breaks the container permanently

- **Public issue.** https://github.com/Blackwellboy/model-serving-minefield/issues/100
- **CONFIRM.** On a pinned Docker engine and Linux host, and with the MOUNT SYNTAX pinned and recorded in both arms, start an otherwise working containerised serve with one bind-mount source path removed, and confirm the daemon creates that path as an empty directory and starts the container rather than refusing, with the resulting failure arriving minutes into load as a checkpoint or model error rather than as a mount error. The syntax is a required condition, not a detail: `-v`/`--volume`, and a Compose `volumes:` short entry, are documented to create a missing host path, while `--mount` is documented to fail with `bind source path does not exist`. A protocol that leaves it unrecorded can return opposite verdicts on the same engine and host. Separately, take a serve whose bind-mount source is a FILE, delete it, start the container, and confirm the source is recreated as a directory, that the container then fails immediately and permanently with a "not a directory" error, and that it keeps failing after the original file is restored. Require the second failure's cause to be present in the container's `.State.Error` while `docker logs` is empty or uninformative, and record the Docker engine version, because the fabrication behaviour is not established across versions.
- **REFUTE.** On the pinned creating syntax, the daemon refuses to start with a missing bind-mount source, or names the mount rather than the model when the serve fails. A `--mount` reproducer failing is NOT a refutation, since that syntax is documented to error and the entry does not claim otherwise. the deleted file mount is recreated as a file, or the container recovers once the file is restored; the immediate failure's cause is available in `docker logs` as well as `.State.Error`; or the late failure is attributable to an independent checkpoint fault rather than to the fabricated empty directory.
- **Boundary.** This is a container-runtime behaviour rather than a defect in any serving engine, and the entry claims only that it is routinely misattributed to one. It does not establish that every Docker version or every container runtime fabricates the path, so the portable defence is a preflight asserting mount CONTENTS, not reliance on a particular version refusing. An existence check is not such a preflight, because the first failure guarantees existence. Choosing `--mount` over `-v` is a second and narrower defence: it converts the late, misattributed failure into an immediate and correctly named one, but it does not help anywhere the short Compose syntax is already in use.
