...
| A multi-node weight load dies at a 10-minute NCCL watchdog and leaves a rank wedged until a power cycle | The parallel shard loader runs collectives during load, and its transient peak exceeds the unified-memory envelope | [131](traps/runtime/131-parallel-loader-collectives-wedge-uma.md) | contributor-measured, conditions as reported |
| A fresh boot looks wedged: GPU busy, token counters at zero, a request that normally takes seconds running 10+ min | The first request after boot pays kernel JIT compilation; counters only advance on completion | [132](traps/evaluation/132-first-request-after-boot-pays-jit.md) | contributor-measured, conditions as reported |
| A staged model fails to resolve offline on one or more nodes, or a sha-pinned fetch leaves the bare name unresolvable | A stray byte in the HF hub refs file breaks offline revision resolution | [133](traps/versioning/133-hf-refs-file-breaks-offline-resolution.md) | contributor-measured, conditions as reported |
