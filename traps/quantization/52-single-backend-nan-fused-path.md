# Trap 52: single-backend NaN is a backend bug, not a quantization-quality result

**Found by TheTom.**

**Status: reproduced here.** Perplexity run on three backends with the same file; the fix is an
opt-in toggle we added and measured.
