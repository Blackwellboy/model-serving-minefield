# Trap 57: the context length it *supports* is not the context length it was *trained* at

**Found by TheTom.**

**Status: reproduced here** (two models scored on the same long-context retrieval battery at the
same
length; raw scores held outside the tree; can be produced on request, per
