...
| The launcher prints a container ID for every worker and the workers never start | JSON CLI args re-split by the remote shell in an SSH fanout; docker creates the container and leaves it in Created | [121](traps/runtime/121-ssh-fanout-mangles-json-args.md) | contributor-measured, conditions as reported |
| A 2-bit expert prepack succeeds on a renamed checkpoint and the served model is degraded | Format detected from tensor names rather than dtype, so fp8 data can be routed through the mxfp4 decoder | [122](traps/quantization/122-prepack-format-detected-by-tensor-name.md) | under test |
