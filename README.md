...
| Tool call comes back empty and the agent retries the same call | Template gates parameters on `is mapping`; replayed JSON-string args render an empty call | [43](traps/template/43-tool-args-string-not-mapping.md) | reproduced here |
| You raised the ceiling and a few percent of turns still come back empty | Reasoning length is right-skewed; no fixed ceiling closes the tail | [44](traps/evaluation/44-retry-on-truncation-residual-tail.md) | reproduced here |
| Dequant reads cosine 0.92 and the model answers "9.9 vs 9.11" as "9 and 9" | Scales stored linear, decoded swizzled | [45](traps/quantization/45-fp4-dequant-scale-swizzle-layout.md) | reproduced here |
| A KV quant pair "is slow": prefill drops ~20x, decode halves, no error | Kernels for that pair were never compiled; silent CPU fallback | [46](traps/quantization/46-fa-all-quants-cpu-fallback.md) | reproduced here |
| Decode 2.5x below expectation, GPU at 98% utilization under 50% TDP | The deployed binary predates the arch-native kernel; the fix was already in the branch | [47](traps/versioning/47-stale-build-missing-arch-kernel.md) | reproduced here |
| Agent turns never get faster as the conversation grows | Prefix caching auto-disabled for hybrid recurrent architectures | [48](traps/runtime/48-prefix-caching-autodisabled-hybrid.md) | reproduced here |
| Every request takes ~30s regardless of prompt size or cache state | Dead IPv6 route on a dual-stack mDNS endpoint; invisible server-side | [49](traps/routing/49-dual-stack-mdns-latency-tax.md) | reproduced here |
| A large clean performance gap that shrinks when the harness is fixed | The prompt never tokenized to the length the table claims | [50](traps/evaluation/50-prompt-not-tokenized-to-target.md) | reproduced here |
| "Layer N exploded" against a reference implementation | Dump index convention off by one, plus pre-norm compared against post-norm | [51](traps/evaluation/51-hidden-state-dump-convention.md) | reproduced here |
| Perplexity is NaN on one backend and clean on the others, same file | A fused rotate-activation path NaNs on one FFN shape | [52](traps/quantization/52-single-backend-nan-fused-path.md) | reproduced here |
| The fast configuration turns out to be fast because it is wrong | Optimizations that remove required work improve throughput and destroy output | [53](traps/evaluation/53-speed-measured-on-a-broken-config.md) | reproduced here |
| A config edit has no effect and the flag "does not work" | Restart reported success; a stale process kept the port and the old config | [54](traps/runtime/54-config-edit-never-took-effect.md) | reproduced here |
| A speedup that disappears on a build without the feature | Run order, warm caches, or cross-session baseline drift | [55](traps/evaluation/55-run-order-and-warm-cache-artifacts.md) | reproduced here |
| KV-quant quality delta is implausibly small | Single-pass prefill never reads the quantized cache back | [56](traps/quantization/56-kv-quant-unread-without-chunked-prefill.md) | reproduced here |
| A long-context model scores badly at its advertised length | Advertised context, served context, and trained context are three different numbers | [57](traps/evaluation/57-supported-context-is-not-trained-context.md) | reproduced here |
...
- 2026-07-28: traps [43](traps/template/43-tool-args-string-not-mapping.md) through [57](traps/evaluation/57-supported-context-is-not-trained-context.md), contributed by [@TheTom](https://github.com/TheTom): fifteen entries from a serving and benchmarking notebook, with eight runnable checks. Four are silent-fallback traps (a KV quant pair whose kernels were never compiled, a binary that predates its own arch kernel, prefix caching auto-disabled for hybrid architectures, a restart that reported success while a stale process kept the port); five are measurement traps that each invalidated a number of ours (a prompt that never tokenized to its stated length, a speedup that reproduced on a build without the feature, a throughput figure that was fast because the dequant was wrong, a parity explosion that was a dump-convention mismatch, KV-quant quality measured without chunked prefill so the quantized cache was never read back). [44](traps/evaluation/44-retry-on-truncation-residual-tail.md) sits under trap 12's 2026-07-28 procedure rather than beside it: it supplies the reason step 4 has to be a distribution (reasoning length is right-skewed within one model and one task, so 9.1% of turns were still empty after the ceiling was raised to 5120) and the automated form of step 3, plus a second per-size data point for trap 22 at 26% against 2.7% on an identical 2560 ceiling. Raw is held outside the tree on all fifteen and can be produced on request.
