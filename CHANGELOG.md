...
- Public mining note: private evidence archive *(private evidence archived)* (W32 under test; PR#12 map/UDP fixes remain engineering-only).
...
- prepack name/format collision candidate *(private evidence archived)*:
...
- Muse Glimmer 30B campaign (llama.cpp + DFlash): **no new trap IDs.** Existing-owner extensions and one mining note only.
  - Mining: 2026-08-11-muse-glimmer-30b-reasoning-control-and-stack.md *(private evidence archived)* - card system-low dual LOW+HIGH render vs clean kwargs-low; full EvalPlus coding cost **not demonstrated**; ops latency/reasoning-length reduction bounded; claim boundaries for Qwen practical lane and DFlash.
...
- **Traps [109](traps/quantization/109-requant-skips-draft-layer-experts.md) through [111](traps/evaluation/111-greedy-spec-decode-medians-are-a-content-lottery.md): a requant whose exclusion list broke its own drafter, and the two measurement traps stepped in while proving it.** The source is a first-party NVFP4 routed-experts requant of DeepSeek-V4-Flash-0731, served by a vLLM-derived NVFP4 fused-MoE build at tensor parallel 2 on 2x RTX PRO 6000 Blackwell. All three land at **measured here, raw not published**, and each names what a stranger can check without us. [109](traps/quantization/109-requant-skips-draft-layer-experts.md) is the one to read: the requant excluded the MTP layers, their expert weights stayed in the source MXFP4 format, the NVFP4 fused-MoE path loaded them anyway without a single log line, and cumulative draft acceptance sat at **14.7%** against the vendor's published 50.8% while speculation lost to its own no-speculation baseline. The recovery is a closed-form MXFP4 to NVFP4 recast of the MTP experts, 603,979,776 of 603,979,776 blocks bit-identical, and the recast checkpoint with its conversion report is public on the hub, which is what makes the layout half of the entry checkable. The recovery was itself nearly rolled back over a drafter sampling method garble, and the entry records the inversion against [62](traps/runtime/62-spec-decode-garble-under-wrong-drafter-config.md): the method that garbles on this lane is the method that stabilised that one. [110](traps/evaluation/110-unscreened-bench-on-a-shared-endpoint.md) is why the first benchmark of the fix read 41 to 77 tok/s on a lane that cleanly does 167 to 302: co-tenant traffic inside the timing window, screened afterwards by two reads of `/metrics` per timed run, 12 of 12 windows clean, and it scopes the co-tenancy null in [95](traps/runtime/95-two-gpu-co-tenancy-does-not-perturb-either-lane.md): sharing a host was not the hazard, sharing an endpoint is. [111](traps/evaluation/111-greedy-spec-decode-medians-are-a-content-lottery.md) is the variance that survives screening: two clean suites, same config, same twelve prompts, medians 242.5 and 199.9 tok/s, because greedy tie-breaks resample the generated text across sessions and acceptance prices the text, roughly linearly, 2.8 accepted per step decoding at about 168 tok/s and 5.6 at about 302. Its fix is quoting the range and the acceptance profile next to any speculative median, and refusing cross-session A/Bs below about a 20% effect. Registry count 108 to 111.
...
- **Trap [33](traps/routing/33-moe-inference-topk-expansion-tax.md) promoted to reported by others + reproduced here**, on a quantised build. The finder's numbers are all bf16 under HF transformers; ours are NVFP4 on vLLM, which under our own different-quant-different-unit rule left the question open rather than settled. It survives at roughly the reported magnitude: monotone across k in {8, 16, 24, 32}, two scoring protocols, two independent passes each, with the pre-registered primary contrast at **-4.50 points** paired at n=600, discordant 37/10, exact McNemar **p = 9.8e-05**, and an independent replicate at -4.00 points, 37/13, p = 0.000936. The same run re-measured our own noise: all four same-k restart pairs landed inside the plus-or-minus 1.3 point band (largest 0.83), every raised-k contrast outside it. The choice-logprob arms, which are the finder's own protocol, came back at -3.17 and -3.67 against his reported -3.67. Every published figure was re-derived from the answer sheets before publication by a checker written separately from the analyser that produced them. Raw is **not** shipped: [MAINTAINING](MAINTAINING.md#shipping-raw-data-in-the-repo) reserves in-repo raw for calibration constants other entries cite, and applying that rule to our own result rather than making an exception for it is the point. The runnable scripts do ship. Writeup *(private evidence archived)*. **Second first-party confirmation of an external contributor's finding** in this registry, after trap [35](traps/evaluation/35-identical-weights-do-not-score-identically.md); the **Found by** line did not move in either case.
...
- **R2-39 settled on the stack it was scoped to.** Refuted as stated: empty content tracks tools alone, in both thinking states, and every empty response carried a tool call. Not a defect, a harness reading `content` and ignoring `tool_calls`. Also recorded: SGLang is not infeasible *(private evidence archived)* on aarch64 GB10 CUDA 13, which is the opposite of the expected result and is why the note exists.
...
- Verification queue recorded in
  private evidence archive *(private evidence archived)*: trap 33 is
...
- New private evidence archive *(private evidence archived)* area: verification notes on mined candidates that
