...
- **Trap [109](traps/template/109-inline-system-role-is-not-a-stable-contract.md) turns inline-system handling into a rendered-evidence contract.** Exact pinned Jinja execution classifies GLM-5.1, GLM-5.2 and Kimi-K2.6 as `ROLE_MARKED`, while MiniMax-M2.5, M2.7 and M3 are `DROPPED` for both the canonical and post-tool probes. The existing live DeepSeek-V4-Flash `/tokenize` render is `WELDED_TO_USER`. Kimi-K3's pinned CPU-only remote tokenizer executed in an isolated profile, but its untokenized render and token-ID decode differ around structural-token spacing, so the strict result remains `INCONCLUSIVE` and the vLLM endpoint remains `UNDER_TEST`. The source map, pins and loading-path distinction came from [@wqh17101](https://github.com/wqh17101), with explicit publication permission in [vLLM issue #46710](https://github.com/vllm-project/vllm/issues/46710#issuecomment-5131158274); the registry fetched and executed the public artifacts independently. A stdlib-only classifier, contribution schema/example, raw manifest, mutation tests and a narrow pinned-source addendum to [56](traps/template/56-checkpoint-ships-no-chat-template.md) land with it. Registry count 108 to 109; doctor coverage remains 19.
