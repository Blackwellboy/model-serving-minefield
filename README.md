...
Four doors, and which one you want depends on why you are here. All 140
...
- **"What am I running?"** The **[per-stack pages](stacks/)** give you the five
  ...
  **Every stack has a page, including the ones we have never run**:
  ...
  [TensorRT-LLM](stacks/tensorrt-llm.md),
...
- **"What am I seeing?"** The **[symptom table](#find-your-symptom)** is
  directly below, all 140 entries, one row each, sorted by number. It is the
...
for **19 of these 140 entries**, weighted toward reasoning fields, templates and
tool parsing, and a clean run from it says nothing about the other 121. It runs
...
**Things that did not work out** are collected in private evidence archive *(private evidence archived)*: the
...
All 140 entries. If you know what you are running rather than what you are
...
| `reasoning_effort` levels change nothing | Template never reads the parameter | [07](traps/reasoning/07-reasoning-effort-silently-ignored.md) | reported by others + reproduced here |
...
| Concurrent HTTP clients report C4 while batch wall ~scales with C and aggregate tok/s stays flat | Concurrent HTTP clients are not concurrent model execution | [135](traps/evaluation/135-concurrent-http-clients-are-not-concurrent-model-execution.md) | measured here, raw not published |
| Probe traceback is visible but the wrapper records `rc=0` | A log consumer's status replaced the producer's failure | [136](traps/evaluation/136-pipeline-reports-consumer-status-not-producer-failure.md) | contributor-measured, conditions as reported |
| Correct finite kernel output is marked failed because the integer return is nonzero | Dispatch selector mistaken for process exit status | [137](traps/evaluation/137-kernel-selector-return-is-not-process-exit-status.md) | contributor-measured, conditions as reported |
| HTTP 200 / normal finish, but assistant JSON contains duplicated ordinary keys or values | Draftless n-gram prompt lookup duplicated structured-output tokens | [138](traps/evaluation/138-ngram-prompt-lookup-duplicates-structured-output-tokens.md) | contributor-measured, conditions as reported |
| A direct-cable multi-NIC collective times out even though the peer link is healthy | NCCL selected an interface that cannot physically reach that peer on the switchless topology | [139](traps/runtime/139-stock-nccl-round-robins-hcas-to-the-ring-successor.md) | contributor-measured, conditions as reported |
| Sparse-MLA warmup metadata names a request row that the block table does not contain | An invalid request-row selector can silently map to the wrong KV slot or fault in the downstream mapper | [140](traps/memory/140-sparse-mla-warmup-metadata-can-index-nonexistent-request-row.md) | measured here, raw not published |
...
| Container Up and /health look fine while completions fail, or unauthenticated 401 is treated as host down | Process or transport liveness is not authenticated model readiness | [112](traps/runtime/112-process-liveness-is-not-model-readiness.md) | contributor-measured, conditions as reported |
| Same inline message sequence yields ROLE_MARKED, DROPPED, or WELDED_TO_USER prompts across constructors | Inline system role is not a stable render contract | [113](traps/template/113-inline-system-role-is-not-a-stable-contract.md) | reproduced here |
...
| High util and P0, but SM clock/power stuck abnormally low under load; serving and compute both slow until full AC power removal | Persistent DGX Spark / GB10 low-power platform state; nvidia-smi clock lock does not recover it; clocks, power, BF16 and LLM throughput recover together after complete AC removal; post-recovery longevity window healthy without proving permanence | [124](traps/runtime/124-dgx-spark-gb10-stuck-low-power-state-under-load.md) | measured here, raw not published |
...
| Speculative decode enabled, output clean, acceptance far below the vendor's figure, and speculation loses to its own no-spec baseline | The requant's exclusion list left the MTP experts in the source format; they load through the NVFP4 fused-MoE path without error and draft garbage | [109](traps/quantization/109-requant-skips-draft-layer-experts.md) | measured here, raw not published |
...
against 19 of this registry's 140 entries in under a minute:
...
(or could not) promote are recorded in private evidence archive *(private evidence archived)*; a negative is
information too.
...
- 2026-09-02: **traps [136](traps/evaluation/136-pipeline-reports-consumer-status-not-producer-failure.md) and [137](traps/evaluation/137-kernel-selector-return-is-not-process-exit-status.md)** - preserve the producer's pipeline status and distinguish native-extension dispatch selectors from process exit codes (**contributor-measured, conditions as reported**); the agentic research playbook gains implementation-path proof gates. Registry 135→137.
- 2026-08-25: **trap [135](traps/evaluation/135-concurrent-http-clients-are-not-concurrent-model-execution.md) + Trap [134](traps/evaluation/134-link-up-is-not-path-proof-for-the-interface-under-test.md) controlled TRANSPORT addendum** - concurrent HTTP clients are not concurrent model execution (**measured here, raw not published**); Trap 134 gains the positive holds+path-proof TRANSPORT corroboration; offline `concurrency_execution_proof_preflight`. Registry 134→135. Mining note *(private evidence archived)*.
- 2026-07-28: **first published SGLang evidence surfaces**, contributor-measured by [@newageinvestments25-byte](https://github.com/newageinvestments25-byte) on a DGX Spark. A pinned Nemotron NVFP4 control generated first; Laguna S 2.1 loaded its compressed-tensors NVFP4 MoE path and generated a correct first token, refuting Q7 under its pre-registered generation criterion without claiming healthy long-form output. Two 14-request doctor runs confirmed Q8 portability and reproduced SGLang instances inside traps [02](traps/template/02-orphaned-think-close-tag.md), [12](traps/evaluation/12-empty-content-at-token-ceiling.md) and [77](traps/reasoning/77-only-one-request-field-is-validated.md). The same run found and regression-tested a doctor fix: SGLang identifies itself in `/v1/models`, not `/version`. Conditions and bounded disposition *(private evidence archived)*.
...
- 2026-07-27: trap [32](traps/runtime/32-mlx-server-max-tokens-is-a-default-not-a-cap.md): mlx_lm's server `--max-tokens` flag is a per-request default, not a cap; a client can quietly run past it. Same pass landed MLX-scoped sections in six existing entries (mlx_lm now has real coverage in the [per-stack index](models/README.md)) and the new private evidence archive *(private evidence archived)* verification-notes area for candidates that did not or could not promote.
